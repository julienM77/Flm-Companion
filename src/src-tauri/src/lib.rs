...
fn update_tray_menu(
    ...
    let is_dark = app
        ...
        .unwrap_or(false);
    let (play_icon, stop_icon, power_icon, red_icon, green_icon) = get_theme_icons(is_dark);
    ...
    let server_submenu =
        Submenu::with_items(&app, server_text, true, &[&start_i, &stop_i]).unwrap();
    ...
    let settings_i =
        MenuItem::with_id(&app, "settings", &text_settings, true, None::<&str>).unwrap();
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            let is_dark = app
                ...
                .map(|w| matches!(w.theme(), Ok(Theme::Dark)))
                .unwrap_or(false);
            let (play_icon, stop_icon, power_icon, red_icon, _green_icon) =
